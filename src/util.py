...
def load_mean_theta():
    ...
    mean_pose = np.zeros([48])
    mean_pose[:3] = 0  # rotation
    mean_pose[0] = np.pi  # todo check
    mean_pose[3:] = mano_model['hands_mean']

    # mean_shape = mean_values['shape'] # zeroes

    # init scale is 0.9, translation (0,0)
    mean[0] = 0.9

    mean[3:48 + 3] = mean_pose[:]
    # mean[58:] = mean_shape[:]
...
def batch_orth_proj(X, camera):
    ...
    X_trans = ortho_proj + translation
    shape = X_trans.shape

    return (scale * X_trans.view(shape[0], -1)).view(shape)
...
def flip_image(src_image, kps):
    ...
    if kps is not None:
        kps[:, 0] = w - 1 - kps[:, 0]
        # kp_map = [5, 4, 3, 2, 1, 0, 11, 10, 9, 8, 7, 6, 12, 13] # reorder joints
        # kps[:, :] = kps[kp_map]
...
def draw_lsp_14kp__bone(src_image, pts):
    ...
    for pt in pts:
        if pt[2] > 0.2:
            cv2.circle(src_image, (int(pt[0]), int(pt[1])), 4, (0, 255, 255), -1)
...
def convert_image_by_pixformat_normalize(src_image, pix_format, normalize):
    ...
    if normalize:
        src_image = normalize_image(src_image)
